 ...
 */
function portfolio_web_sanitize_social_data( $input ){
	...
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				...
				elseif ( $key == 'checkbox' ){
					$input_decoded[$boxes][$key] = portfolio_web_sanitize_checkbox( $value );
                }
                else{
 ...
 * @return string json encoded
 */
function portfolio_web_sanitize_slider_data( $input ){
	$input_decoded = json_decode( $input, true );
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				if( $key == 'selectpage'){
					$input_decoded[$boxes][$key] = portfolio_web_sanitize_page( $value );
				}
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Portfolio_Web_Customize_Icons_Control' )):
	/**
	 ...
	 *
	 */
	class Portfolio_Web_Customize_Icons_Control extends WP_Customize_Control {
            ...
            <label>
                ...
                <span class="at-customize-icons">
                    ...
                    <span class="icon-toggle">
                        <?php echo ( empty( $value )? esc_html__('Add Icon','portfolio-web'): esc_html__('Edit Icon','portfolio-web') ); ?>
                        <span class="dashicons dashicons-arrow-down"></span>
                    </span>
                    <span class="icons-list-wrapper hidden">
                        <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','portfolio-web')?>">
	                    <?php
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Portfolio_Web_Repeater_Control' )):
	/**
	 ...
	 *
	 */
	class Portfolio_Web_Repeater_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		...
		public function enqueue(){
			...
			wp_enqueue_style( 'portfolio-web-customizer-repeater-style', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.css', array(), '3.3.6' );
			wp_enqueue_script( 'portfolio-web-customizer-repeater-script', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.js', array('jquery', 'jquery-ui-draggable' ), '1.0.0', true  );
		}
                    ...
                    <div class="repeater-fields hidden">
								...
								switch ( $field['type'] ) {
									...
									case 'icons':
                                        ...
                                        <span class="at-customize-icons">
                                            ...
                                            <span class="icon-toggle">
                                                <?php esc_html_e('Add Icon','portfolio-web'); ?>
                                                <span class="dashicons dashicons-arrow-down"></span>
                                            </span>
                                            <span class="icons-list-wrapper hidden">
                                                <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','portfolio-web')?>">
									...
									case 'image':
                                        ...
                                        <span class="img-preview-wrap" <?php echo  $portfolio_web_display_none ; ?>>
                                            <img class="widefat" src="<?php echo esc_url( $new_value ); ?>" alt="<?php esc_attr_e( 'Image preview', 'portfolio-web' ); ?>"  />
                        ...
                        <div class="clearfix repeater-footer">
                            <a class="repeater-field-remove" href="#remove">
								<?php esc_html_e( 'Delete', 'portfolio-web' ); ?>
                            </a>
	                        <?php esc_html_e( '|', 'portfolio-web' ); ?>
                            <a class="repeater-field-close" href="#close">
								<?php esc_html_e( 'Close', 'portfolio-web' ); ?>
                            </a>
                        </div>
                    </div>
                </li>
									...
									switch ( $field['type'] ) {
										...
										case 'icons':
                                            ...
                                            <span class="at-customize-icons">
                                                ...
                                                <span class="icon-toggle">
                                                    <?php echo ( empty( $new_value )? esc_html__('Add Icon','portfolio-web'): esc_html__('Edit Icon','portfolio-web') ); ?>
                                                    <span class="dashicons dashicons-arrow-down"></span>
                                                </span>
                                                <span class="icons-list-wrapper hidden">
                                                    <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','portfolio-web');?>">
