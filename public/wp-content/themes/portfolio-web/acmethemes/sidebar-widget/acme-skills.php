<?php
/**
 * Class for adding Skills Section Widget
 *
 * @package Acme Themes
 * @subpackage Portfolio Web
 * @since 1.0.0
 */
if ( ! class_exists( 'Portfolio_Web_Skills' ) ) {

	class Portfolio_Web_Skills extends WP_Widget {
		...
		function __construct() {
			...
			/*Base ID of your widget*/
				...
				esc_html__( 'AT Skills Section', 'portfolio-web' ),
				/*Widget description*/
				array(
		...
		public function form( $instance ) {
			...
			$at_all_page_items  = $instance['at_all_skills_items'];
            ...
            <div class="at-repeater">
				...
				if  ( is_array( $at_all_page_items ) && count( $at_all_page_items ) > 0 ){
					foreach ( $at_all_page_items as $skills_detail ){

						$repeater_skills_page_id  = $this->get_field_id( 'at_all_skills_items') .$total_repeater.'page_id';
						$repeater_skills_page_name  = $this->get_field_name( 'at_all_skills_items' ).'['.$total_repeater.']['.'page_id'.']';

						$repeater_skill_percentage_id  = $this->get_field_id( 'at_all_skills_items') .$total_repeater.'skill_percentage';
						$repeater_skill_percentage_name  = $this->get_field_name( 'at_all_skills_items' ).'['.$total_repeater.']['.'skill_percentage'.']';
                        ...
                        <div class="repeater-table">
                            <div class="at-repeater-top">
                                ...
                                <div class="at-repeater-title">
                                    <h3><?php esc_html_e( 'Skill', 'portfolio-web' )?><span class="in-at-repeater-title"></span></h3>
                                </div>
                            </div>
                            <div class='at-repeater-inside hidden'>
                                ...
                                <p>
                                    <label><?php esc_html_e( 'Enter Skills % Number', 'portfolio-web' ); ?></label>
                                    <input type="number" class="widefat" name="<?php echo esc_attr( $repeater_skill_percentage_name ); ?>" id="<?php echo esc_attr( $repeater_skill_percentage_id ); ?>" value="<?php echo absint( $skills_detail['skill_percentage'] ); ?>" />
                                </p>
				...
				$repeater_skills_page_id  = $this->get_field_id( 'at_all_skills_items') .$coder_repeater_depth.'page_id';
				$repeater_skills_page_name  = $this->get_field_name( 'at_all_skills_items' ).'['.$coder_repeater_depth.']['.'page_id'.']';

				$repeater_skill_percentage_id  = $this->get_field_id( 'at_all_skills_items') .$coder_repeater_depth.'skill_percentage';
				$repeater_skill_percentage_name  = $this->get_field_name( 'at_all_skills_items' ).'['.$coder_repeater_depth.']['.'skill_percentage'.']';
                ...
                <script type="text/html" class="at-code-for-repeater">
                    <div class="repeater-table">
                        ...
                        <div class='at-repeater-inside hidden'>
                            ...
                            <p>
                                <label><?php esc_html_e( 'Enter Skills % Number', 'portfolio-web' ); ?></label>
                                <input type="number" class="widefat" name="<?php echo esc_attr( $repeater_skill_percentage_name ); ?>" id="<?php echo esc_attr( $repeater_skill_percentage_id ); ?>" />
                            </p>
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'column_number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'column_number' ) ); ?>">
					<?php
					$portfolio_web_skills_column_numbers = portfolio_web_widget_column_number();
					foreach ( $portfolio_web_skills_column_numbers as $key => $value ) {
						?>
		...
		public function update( $new_instance, $old_instance ) {
			...
			/*updated code*/
			$at_all_page_items_data = array();

			if( isset($new_instance['at_all_skills_items'] )){
				$at_all_page_items               = $new_instance['at_all_skills_items'];

				if  ( is_array($at_all_page_items) && count($at_all_page_items) > 0 ){
					foreach ( $at_all_page_items as $boxes => $box ){
						foreach ( $box as $key => $value ){
							$at_all_page_items_data[$boxes][$key] = absint( $value );
						}
					}
				}
			}

			$instance[ 'at_all_skills_items' ]     = $at_all_page_items_data;
		...
		public function widget( $args, $instance ) {
			...
			?>
            <section id="<?php echo esc_attr( $unique_id ); ?>" class="at-widgets at-skills <?php echo $bg_gray_class;?> <?php echo $bg_image_class;?>" style="<?php echo $bg_image_style; ?>">
                <div class="container">
					<?php
					if( ! empty( $title ) ){
                    ...
                    <div <?php echo $div_attr;?>>
						...
						if  ( is_array($at_all_page_items) && count($at_all_page_items) > 0 ){
							foreach ( $at_all_page_items as $skills ){
								if( isset( $skills['page_id'] ) && !empty( $skills['page_id'] ) ){
									$page_id = $skills['page_id'];

									$post_in[] = $page_id;
									$menu_other_details[$page_id]['skill_percentage'] = $skills['skill_percentage'];
								}
							}
						}
						if( !empty( $post_in ) && is_array( $post_in)) :
							...
							$skills_query = new WP_Query( $portfolio_web_post_in_page_args );

							/*The Loop*/
							if ( $skills_query->have_posts() ):
								$portfolio_web_featured_index = 1;
								while ( $skills_query->have_posts() ):$skills_query->the_post();
									$skill_percentage  = $menu_other_details[get_the_ID()]['skill_percentage'];
									...
									?>
                                    <div class="<?php echo esc_attr( $portfolio_web_list_classes ); ?> <?php echo esc_attr( $animation ); ?>">
                                        <span class="chart" data-percent="<?php echo absint($skill_percentage); ?>">
	...
	} // Class Portfolio_Web_Skills ends here
}